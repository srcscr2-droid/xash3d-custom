...
#define FILE_BUFF_SIZE		(1<<14)	// 16384: 8x larger than old 2048, fewer refill syscalls
...
struct file_s
{
	...
	qboolean	need_seek;			// true when OS fd pos is out of sync (after write/explicit seek)
						// Contents buffer
...
file_t *FS_OpenPackedFile( pack_t *pack, int pack_ind )
{
	...
	file->position = 0;
	file->ungetc = EOF;
	file->need_seek = false;	// lseek to offset was done above by pack open
...
fs_offset_t FS_Write( file_t *file, const void *data, size_t datasize )
{
	...
	// After a write, OS fd pos has advanced; next read refill must re-sync
	file->need_seek = true;
...
fs_offset_t FS_Read( file_t *file, void *buffer, size_t buffersize )
{
	...
	if( buffersize > sizeof( file->buff ) / 2 )
	{
		...
		// Only seek if position is out of sync (e.g. after write or explicit seek)
		if( file->need_seek )
		{
			lseek( file->handle, file->offset + file->position, SEEK_SET );
			file->need_seek = false;
		}
	...
	else
	{
		...
		// Only seek if position is out of sync
		if( file->need_seek )
		{
			lseek( file->handle, file->offset + file->position, SEEK_SET );
			file->need_seek = false;
