...
void HTTP_Run( void )
{
	...
	while ( ( res = recv( curfile->socket, buf, sizeof( buf ), 0 ) ) > 0 ) // if we got there, we are receiving data
	{
		...
		{
			// Guard against overflow: only copy what fits in the header buffer
			int space = (int)sizeof( http.buf ) - http.header_size - 1;
			if( space <= 0 )
			{
				MsgDev( D_ERROR, "HTTP: response header too large, aborting\n" );
				HTTP_FreeFile( curfile, true );
				return;
			}
