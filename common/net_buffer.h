...
// Byte-write functions — trivial wrappers now inlined in net_buffer.h
// Non-trivial ones remain here:
...
// Byte-read functions
// These trivial one-liners are inlined here to avoid function-call overhead;
// they are called hundreds of times per network message.
static inline int BF_ReadChar( sizebuf_t *bf )
{
	return BF_ReadSBitLong( bf, sizeof( char ) << 3 );
}
static inline int BF_ReadByte( sizebuf_t *bf )
{
	return BF_ReadUBitLong( bf, sizeof( byte ) << 3 );
}
static inline int BF_ReadShort( sizebuf_t *bf )
{
	return BF_ReadSBitLong( bf, sizeof( short ) << 3 );
}
static inline int BF_ReadWord( sizebuf_t *bf )
{
	return BF_ReadUBitLong( bf, sizeof( word ) << 3 );
}
static inline int BF_ReadLong( sizebuf_t *bf )
{
	return BF_ReadSBitLong( bf, sizeof( int ) << 3 );
}
// Non-trivial read functions — remain in net_buffer.c
...
// Byte-write inlines — same rationale
static inline void BF_WriteChar( sizebuf_t *bf, int val )
{
	BF_WriteSBitLong( bf, val, sizeof( char ) << 3 );
}
static inline void BF_WriteByte( sizebuf_t *bf, int val )
{
	BF_WriteUBitLong( bf, val, sizeof( byte ) << 3 );
}
static inline void BF_WriteShort( sizebuf_t *bf, int val )
{
	BF_WriteSBitLong( bf, val, sizeof( short ) << 3 );
}
static inline void BF_WriteWord( sizebuf_t *bf, int val )
{
	BF_WriteUBitLong( bf, val, sizeof( word ) << 3 );
}
static inline void BF_WriteLong( sizebuf_t *bf, int val )
{
	BF_WriteSBitLong( bf, val, sizeof( int ) << 3 );
}
// Non-trivial writes — remain in net_buffer.c
void BF_WriteCoord( sizebuf_t *bf, float val );
void BF_WriteFloat( sizebuf_t *bf, float val );
void BF_WriteVec3Coord( sizebuf_t *bf, const float *fa );
