...
void R_ChangeDisplaySettingsFast( int width, int height )
{
	...
	// Proper aspect detection: wideScreen is true when not 4:3 or 5:4
	glState.wideScreen = ( width * 3 != height * 4 ) && ( width * 4 != height * 5 );

	// Physical screen dimensions don't change in a fast switch — preserve them.
	// Only update if not yet initialised (e.g. very first call before VID_SetMode).
	if( !glState.screenWidth || !glState.screenHeight )
	{
		glState.screenWidth  = width;
		glState.screenHeight = height;
	}
...
rserr_t R_ChangeDisplaySettings( int width, int height, qboolean fullscreen )
{
	...
	// Always record the true physical surface size so that R_Set2DMode can
	// set a viewport that covers the whole display (important for r_scale).
	Android_GetScreenRes( &screenW, &screenH );
	glState.screenWidth  = screenW;
	glState.screenHeight = screenH;

	// If the caller passed 0x0 (e.g. from the internal resize path) fall back
	// to the physical screen resolution as the render resolution too.
	if( width <= 0 || height <= 0 )
	{
		width  = screenW;
		height = screenH;
	...
	// wideScreen: true when render resolution is NOT 4:3 or 5:4
	glState.wideScreen = ( width * 3 != height * 4 ) && ( width * 4 != height * 5 );
...
{
	// Pass 0,0 to tell R_ChangeDisplaySettings to fall back to the physical
	// EGL surface dimensions.  Custom render resolutions (r_scale) must go
	// through R_ChangeDisplaySettingsFast instead.
	MsgDev( D_NOTE, "VID_SetMode: resetting to physical screen resolution\n" );
