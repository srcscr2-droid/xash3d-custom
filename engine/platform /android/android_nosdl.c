...
DECLARE_JNI_INTERFACE( void, nativeString, jobject string )
{
	...
	Android_Lock();
	// Fix: strncat's n must be remaining space, not total buffer size.
	// Using total size here allowed overflow when inputtext was already partially filled.
	{
		size_t already = strlen( events.inputtext );
		size_t remaining = sizeof( events.inputtext ) - already - 1;
		if( remaining > 0 )
			strncat( events.inputtext, str, remaining );
	}
	Android_Unlock();
