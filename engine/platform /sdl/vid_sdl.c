...
void R_ChangeDisplaySettingsFast( int width, int height )
{
	...
	// Update centre positions from the NEW dimensions, not the stale glState.
	...
	if( glState.width != width || glState.height != height )
	{
		...
		// Also keep physical screen dims in sync (no r_scale on SDL path here)
...
rserr_t R_ChangeDisplaySettings( int width, int height, qboolean fullscreen )
{
	...
	// On SDL the surface always matches the logical window size — no separate
	// physical vs. render resolution concept unless r_scale is used externally.
	glState.screenWidth  = width;
	glState.screenHeight = height;
...
qboolean VID_SetMode( void )
{
	...
	else if( vid_mode->integer >= 0 && vid_mode->integer < num_vidmodes )
	{
		// Always use the vidmode table when a valid index is set.
		// The old code required vid_mode->modified here, which meant that
		// after a restart (where the cvar is read from config but not
		// "newly changed"), the table was skipped and scr_height->integer
		// was used instead — causing the 800x600 → 800x599 quirk.
		iScreenWidth  = vidmode[vid_mode->integer].width;
		iScreenHeight = vidmode[vid_mode->integer].height;
		vid_mode->modified = false; // acknowledge
	}
	else
	{
