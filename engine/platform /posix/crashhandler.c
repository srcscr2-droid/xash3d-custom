...
#error "Unknown arch!!!"
	...
	len = Q_snprintf( message, sizeof( message ), "Ver: Xash3D-NG " XASH_VERSION " (build %i-%s, %s-%s)\n", Q_buildnum(), Q_buildcommit(), Q_buildos(), Q_buildarch() );
		...
		len += Q_snprintf( message + len, sizeof( message ) - len, "Sys_Crash: signal %d, err %d with code %d at %p\n", signal, si->si_errno, si->si_code, si->si_addr );
	#else
		len += Q_snprintf( message + len, sizeof( message ) - len, "Sys_Crash: signal %d, err %d with code %d at %p %p\n", signal, si->si_errno, si->si_code, si->si_addr, si->si_ptr );
	#endif
	write(2, message, len);
