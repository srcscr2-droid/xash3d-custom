...
convar_t	*sv_hitbox_expand;	// absolute per-axis hitbox expansion in world units
convar_t	*default_hitbox;	// 0=full visual model hull  1=legacy per-bone hitboxes
...
void SV_Init( void )
{
	...
	sv_hitbox_expand = Cvar_Get( "sv_hitbox_expand", "0", CVAR_ARCHIVE, "expand all hitbox planes by this many world units (0=off). Larger = more forgiving registration" );
	default_hitbox = Cvar_Get( "default_hitbox", "0", CVAR_ARCHIVE | CVAR_LATCH, "0: full visual model hitbox (covers whole model). 1: legacy per-bone hitboxes. Restart required when changing." );
