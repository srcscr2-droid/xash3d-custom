...
{
	int		isPointTrace;
	float		scale = 0.0f;	// default: exact hitbox fit (no expansion)
	hull_t		*hull = NULL;
	vec3_t		size;
	model_t		*mod;
	...
	if( VectorIsNull( size ) && !( svgame.globals->trace_flags & FTRACE_SIMPLEBOX ))
	{
		...
		if( ent->v.flags & ( FL_CLIENT|FL_FAKECLIENT ))
		{
			...
			{
				// sv_clienttrace 0: use bounding box, no hitbox trace
				isPointTrace = false;
			}
			else
			{
				// sv_clienttrace scales the 1-unit point into a small volume
		...
		{
			// Non-client studio entity point trace: give it a 1-unit volume
			// so the DotProductFabs expansion in Mod_HullForStudio is non-zero
			// and the planes actually bound a solid region.
			VectorSet( size, 1.0f, 1.0f, 1.0f );
			scale = 0.5f;
		}
	}
	...
	if( mod->flags & STUDIO_TRACE_HITBOX || isPointTrace )
	{
		...
		// --------------------------------------------------------
		// default_hitbox == 0: full visual model hull (default)
		// Use the sequence bounding box (computed from all vertices
		// in the animation) as a single world-space AABB.  This means
		// a bullet that visually hits the model will ALWAYS register,
		// even if the internal per-bone boxes don't cover that pixel.
		//
		// default_hitbox == 1: legacy per-bone hitboxes (requires restart)
		// --------------------------------------------------------
		if( !default_hitbox || !default_hitbox->integer )
		{
			studiohdr_t *pstudio = Mod_Extradata( mod );
			if( pstudio && pstudio->numseq > 0 )
			{
				mstudioseqdesc_t *pseqdesc;
				vec3_t wmin, wmax;
				matrix4x4 worldmat;
				float expand = sv_hitbox_expand ? sv_hitbox_expand->value : 0.0f;
				int seq = ent->v.sequence;

				if( seq < 0 || seq >= pstudio->numseq ) seq = 0;
				pseqdesc = (mstudioseqdesc_t *)((byte *)pstudio + pstudio->seqindex) + seq;

				// Transform model-space visual bounds → world-space AABB.
				// Matrix uses entity angles + origin so the box matches what
				// the player actually sees on screen.
				Matrix4x4_CreateFromEntity( worldmat, ent->v.angles, ent->v.origin, 1.0f );
				World_TransformAABB( worldmat, pseqdesc->bbmin, pseqdesc->bbmax, wmin, wmax );

				// Optional absolute expansion (sv_hitbox_expand units outward)
				if( expand > 0.0f )
				{
					wmin[0] -= expand; wmin[1] -= expand; wmin[2] -= expand;
					wmax[0] += expand; wmax[1] += expand; wmax[2] += expand;
				}

				// Build a single hull with 6 axis-aligned world-space planes.
				// The clipnode tree in studio_clipnodes treats even planes as
				// "outside if t1 >= 0" and odd as "outside if t1 < 0".
				// For both cases the normal is the positive axis and dist is
				// the face distance:
				//   even (max face): normal=+axis, dist=wmax[axis]  → outside when p > max
				//   odd  (min face): normal=+axis, dist=wmin[axis]  → outside when p < min
				studio_planes[0].type   = PLANE_X;
				studio_planes[0].normal[0] = 1.0f; studio_planes[0].normal[1] = 0.0f; studio_planes[0].normal[2] = 0.0f;
				studio_planes[0].dist   = wmax[0];

				studio_planes[1].type   = PLANE_X;
				studio_planes[1].normal[0] = 1.0f; studio_planes[1].normal[1] = 0.0f; studio_planes[1].normal[2] = 0.0f;
				studio_planes[1].dist   = wmin[0];

				studio_planes[2].type   = PLANE_Y;
				studio_planes[2].normal[0] = 0.0f; studio_planes[2].normal[1] = 1.0f; studio_planes[2].normal[2] = 0.0f;
				studio_planes[2].dist   = wmax[1];

				studio_planes[3].type   = PLANE_Y;
				studio_planes[3].normal[0] = 0.0f; studio_planes[3].normal[1] = 1.0f; studio_planes[3].normal[2] = 0.0f;
				studio_planes[3].dist   = wmin[1];

				studio_planes[4].type   = PLANE_Z;
				studio_planes[4].normal[0] = 0.0f; studio_planes[4].normal[1] = 0.0f; studio_planes[4].normal[2] = 1.0f;
				studio_planes[4].dist   = wmax[2];

				studio_planes[5].type   = PLANE_Z;
				studio_planes[5].normal[0] = 0.0f; studio_planes[5].normal[1] = 0.0f; studio_planes[5].normal[2] = 1.0f;
				studio_planes[5].dist   = wmin[2];

				// studio_hull[0].planes points to studio_planes[0..5]
				studio_hull_hitgroup[0] = 0;	// hitgroup 0 = body
				*numhitboxes = 1;
				return studio_hull;
			}
			// fallthrough to legacy if model data unavailable
		}

		// Legacy path: per-bone hitboxes defined in the MDL file
		if( ent->v.flags & ( FL_CLIENT|FL_FAKECLIENT ))
		{
			studiohdr_t	*pstudio;
			mstudioseqdesc_t	*pseqdesc;
...
void SV_ClipMoveToEntity( edict_t *ent, const vec3_t start, vec3_t mins, vec3_t maxs, const vec3_t end, trace_t *trace )
{
	...
	else
	{
		...
		for( i = 0; i < hullcount; i++ )
		{
			...
			// Only accept this hitbox result when it represents a genuine hit:
			//  • startsolid  — started inside solid (valid physical state)
			//  • fraction < 1.0 — ray actually entered the box
			// Do NOT replace a better earlier hit just because allsolid is set
			// on a hitbox the ray never actually penetrated (fraction == 1.0).
			if( i == 0 || trace_hitbox.startsolid || trace_hitbox.fraction < trace->fraction )
			{
				if( trace->startsolid )
				{
					*trace = trace_hitbox;
				...
				// Only record the hitgroup for boxes that were truly hit
				if( trace_hitbox.startsolid || trace_hitbox.fraction < 1.0f )
					last_hitgroup = i;
			}
		}
