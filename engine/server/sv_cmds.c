...
void SV_ConSay_f( void )
{
	...
	// Use remaining space to avoid overflow — same bug as classic strncat(buf, s, sizeof(buf))
	{
		size_t used = Q_strlen( text );
		if( used < sizeof( text ) - 2 )
		{
			Q_strncat( text, p, sizeof( text ) - used - 2 );  // -2: room for \n and \0
			Q_strncat( text, "\n", sizeof( text ) - Q_strlen( text ) - 1 );
		}
	}
...
{
	Cmd_AddServerCommand( "create_server", SV_CreateServer_f, "start a public multiplayer server: create_server <map> [maxplayers]" );
