...
void Info_Print( const char *s )
{
	...
	{
		o = key;
		while( *s && *s != '\\' && ( o - key ) < MAX_INFO_STRING - 1 )
			*o++ = *s++;
		...
		o = value;
		s++;
		while( *s && *s != '\\' && ( o - value ) < MAX_INFO_STRING - 1 )
			*o++ = *s++;
		*o = 0;
...
char *GAME_EXPORT Info_ValueForKey( const char *s, const char *key )
{
	...
	while( 1 )
	{
		...
		*o = 0;
		...
		while( *s != '\\' && *s != '\n' && *s && ( o - value[valueindex] ) < MAX_INFO_STRING - 1 )
		{
			if( !*s ) return "";
			*o++ = *s++;
		}
...
qboolean GAME_EXPORT Info_RemoveKey( char *s, const char *key )
{
	...
	while( 1 )
	{
		...
		while( *s != '\\' && ( o - pkey ) < MAX_INFO_STRING - 1 )
		{
			if( !*s ) return false;
			*o++ = *s++;
		}
