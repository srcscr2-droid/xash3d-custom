...
static void Mod_LoadSurfaces( const dlump_t *l )
{
	...
	for( i = 0; i < count; i++, in++, out++, info++ )
	{
		...
		// Guard against corrupted BSP: validate indices before dereference
		{
			int planenum = LittleShort( in->planenum );
			int texinfo  = LittleShort( in->texinfo  );

			if( planenum < 0 || planenum >= loadmodel->numplanes )
			{
				MsgDev( D_ERROR, "Mod_LoadSurfaces: surface %i has bad planenum %i (max %i)\n", i, planenum, loadmodel->numplanes );
				out->plane = loadmodel->planes; // point to plane 0 as safe fallback
			}
			else out->plane = loadmodel->planes + planenum;

			if( texinfo < 0 || texinfo >= loadmodel->numtexinfo )
			{
				MsgDev( D_ERROR, "Mod_LoadSurfaces: surface %i has bad texinfo %i (max %i)\n", i, texinfo, loadmodel->numtexinfo );
				out->texinfo = loadmodel->texinfo; // point to texinfo 0 as safe fallback
			}
			else out->texinfo = loadmodel->texinfo + texinfo;
		}
