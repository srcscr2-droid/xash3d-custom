...
mstudiocache_t *Mod_CheckStudioCache( model_t *model, float frame, int sequence, vec3_t angles, vec3_t origin, vec3_t size, byte *pcontroller, byte *pblending )
{
	...
	for( i = 0; i < STUDIO_CACHESIZE; i++ )
	{
		...
		if( pCache->model == model && pCache->sequence == sequence &&
		fabs( pCache->frame - frame ) < 0.001f &&	// epsilon instead of exact ==
...
hull_t *Mod_HullForStudio( model_t *model, float frame, int sequence, vec3_t angles, vec3_t origin, vec3_t size, byte *pcontroller, byte *pblending, int *numhitboxes, edict_t *pEdict )
{
	...
	*numhitboxes = 0; // assume error
	...
	for( i = j = 0; i < mod_studiohdr->numhitboxes; i++, j += 6 )
	{
		...
		float bbmax0 = phitbox[i].bbmax[0] + expand;
		float bbmin0 = phitbox[i].bbmin[0] - expand;
		float bbmax1 = phitbox[i].bbmax[1] + expand;
		float bbmin1 = phitbox[i].bbmin[1] - expand;
		float bbmax2 = phitbox[i].bbmax[2] + expand;
		float bbmin2 = phitbox[i].bbmin[2] - expand;

		// Sanity check: if expand made bbmin > bbmax (degenerate box), clamp
		if( bbmin0 > bbmax0 ) bbmin0 = bbmax0 = (bbmin0 + bbmax0) * 0.5f;
		if( bbmin1 > bbmax1 ) bbmin1 = bbmax1 = (bbmin1 + bbmax1) * 0.5f;
		if( bbmin2 > bbmax2 ) bbmin2 = bbmax2 = (bbmin2 + bbmax2) * 0.5f;

		studio_hull_hitgroup[i] = phitbox[i].group;

		Mod_SetStudioHullPlane( &studio_planes[j+0], phitbox[i].bone, 0, bbmax0 );
		Mod_SetStudioHullPlane( &studio_planes[j+1], phitbox[i].bone, 0, bbmin0 );
		Mod_SetStudioHullPlane( &studio_planes[j+2], phitbox[i].bone, 1, bbmax1 );
		Mod_SetStudioHullPlane( &studio_planes[j+3], phitbox[i].bone, 1, bbmin1 );
		Mod_SetStudioHullPlane( &studio_planes[j+4], phitbox[i].bone, 2, bbmax2 );
		Mod_SetStudioHullPlane( &studio_planes[j+5], phitbox[i].bone, 2, bbmin2 );
