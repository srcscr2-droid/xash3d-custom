...
void NET_SendPacket( netsrc_t sock, size_t length, const void *data, netadr_t to )
{
	...
	{
		// data is a raw network packet (binary), not a string — don't pass to %s
		MsgDev( D_ERROR, "NET_SendPacket ( %d, %zu, [0x%02X...], %i ): bad address type %i\n",
			sock, length, length > 0 ? (unsigned char)data[0] : 0, to.type, to.type );
		return;
	}
