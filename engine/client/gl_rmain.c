...
//	if( RI.params & RP_MIRRORVIEW ) RI.projectionMatrix[0][0] = -RI.projectionMatrix[0][0];
	...
	if( RP_NORMALPASS( ))
	{
		...
		// Scale from logical (scr_width x scr_height) space to actual render
		// (glState.width x glState.height) space so that viewsize and split-screen
		// work correctly under r_scale.
		{
			float sx = (float)glState.width  / (float)(scr_width->integer  ? scr_width->integer  : glState.width);
			float sy = (float)glState.height / (float)(scr_height->integer ? scr_height->integer : glState.height);
			x  = (int)floor( RI.viewport[0] * sx );
			x2 = (int)ceil(( RI.viewport[0] + RI.viewport[2] ) * sx );
			y  = (int)floor( glState.height - RI.viewport[1] * sy );
			y2 = (int)ceil( glState.height - ( RI.viewport[1] + RI.viewport[3] ) * sy );
		}
