...
void R_Set2DMode( qboolean enable )
{
	if( enable )
	{
		...
		// Use physical screen dimensions for 2D viewport.
		// When r_scale is active, glState.width/height is the scaled render
		// resolution, but the 2D layer (HUD, menus, console) must cover the
		// entire physical display, not just the scaled sub-region.
		int sw = glState.screenWidth  ? glState.screenWidth  : glState.width;
		int sh = glState.screenHeight ? glState.screenHeight : glState.height;

		pglViewport( 0, 0, sw, sh );
		pglMatrixMode( GL_PROJECTION );
		pglLoadIdentity();
		pglOrtho( 0, sw, sh, 0, -99999, 99999 );
