...
void Con_DrawSolidConsole( float frac, qboolean fill )
{
	...
	if( host_developer->integer )
	{
		...
		float	fraction;
		// Build the version string once and cache it — it never changes at runtime
		static char cached_build[MAX_STRING];
		if( !cached_build[0] )
			Q_snprintf( cached_build, MAX_STRING, "Xash3D-NG %i/%s build %i %s %s-%s",
				PROTOCOL_VERSION, XASH_VERSION, Q_buildnum( ), Q_buildcommit( ), Q_buildos( ), Q_buildarch( ) );

		memcpy( color, g_color_table[7], sizeof( color ) );

		Con_DrawStringLen( cached_build, &stringLen, &charH );
		...
		Con_DrawString( start, 0, cached_build, color );
