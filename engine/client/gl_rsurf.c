...
typedef struct
{
	...
	int		dirty_height;		// max allocated row used — updated in LM_AllocBlock,
					// avoids O(BLOCK_SIZE) scan in LM_UploadDynamicBlock
...
static int LM_AllocBlock( int w, int h, int *x, int *y )
{
	...
	// keep high-water mark so upload functions skip the O(BLOCK_SIZE) scan
	if( best + h > gl_lms.dirty_height )
		gl_lms.dirty_height = best + h;
...
{
	// Use the cached high-water mark instead of scanning all BLOCK_SIZE rows
	int height = gl_lms.dirty_height;
	if( height <= 0 ) return;
...
static void LM_UploadBlock( qboolean dynamic )
{
	...
	if( dynamic )
	{
		// cached high-water mark — avoids O(BLOCK_SIZE) scan every dlight update
		int height = gl_lms.dirty_height;
