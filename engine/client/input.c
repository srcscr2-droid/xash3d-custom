...
void IN_EngineAppendMove( float frametime, usercmd_t *cmd, qboolean active )
{
	...
	// -------------------------------------------------------
	// Auto bunny-hop
	// When cl_autobhop is on and the player holds +jump, we
	// automatically release and re-press jump every frame they
	// are NOT on the ground.  This gives a perfect-timing hop
	// the instant they land without requiring the player to
	// manually time the button press.
	//
	// Logic:
	//   onground != -1  → player is standing on something
	//   IN_JUMP held    → player is holding the jump button
	//
	// If holding jump but in the air: strip IN_JUMP so the
	// server-side PM_Jump() does not keep trying to jump mid-air
	// (which wastes the queued jump and breaks velocity).
	// When they land (onground != -1) the bit stays set for
	// exactly that one frame → engine executes the jump at the
	// earliest possible moment.
	// -------------------------------------------------------
	if( cl_autobhop && cl_autobhop->integer && ( cmd->buttons & IN_JUMP ) )
	{
		// cl.predicted.onground is updated at end of previous frame
		if( cl.predicted.onground == -1 )
		{
			// Still in the air — strip the jump bit.
			// It will be restored next frame automatically by the
			// game DLL's CL_CreateMove if the player is still
			// holding the key.
			cmd->buttons &= ~IN_JUMP;
		}
		// else: on ground → leave IN_JUMP set → normal jump fires.
	}
