...
void SCR_DrawFPS( int height )
{
	...
	static int maxfps     = 0;
	double newtime;
	static char fpsstring[64];	// cached — rebuilt only when value changes
	static int  last_curfps = -1;	// sentinel for cache invalidation
	...
	if ( newtime >= nexttime )
	{
		...
		framecount = 0;
		last_curfps = -1;	// force string rebuild on rate update
	}
	...
	if ( calc < 1.0f )
	{
		int spf = (int)( 1.0f / calc + 0.5f );
		if( last_curfps != -spf || last_mode != cl_showfps->integer )
		{
			Q_snprintf( fpsstring, sizeof( fpsstring ), "%4i spf", spf );
			last_curfps = -spf;
			last_mode   = cl_showfps->integer;
		}
	...
	else
	{
		...
		// Only rebuild the string when the displayed value actually changed
		if( last_curfps != curfps || last_mode != cl_showfps->integer )
		{
