...
void R_AnimateLight( void )
{
	...
	for( i = 0, ls = cl.lightstyles; i < MAX_LIGHTSTYLES; i++, ls++ )
	{
		...
		if( !ls->length )
		{
			// static full-bright style — only recompute when scale changes
			// (scale is constant within this call, so set once per frame is fine)
		...
		{
			// single-key style never changes — still recalc for scale delta,
			// but skip the modulo and multiply when value is already correct.
			int val = (int)( ls->map[0] * 22 * scale );
			if( RI.lightstylevalue[i] != val )
			{
				RI.lightstylevalue[i] = val;
