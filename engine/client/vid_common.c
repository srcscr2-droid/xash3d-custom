...
static void GL_SetDefaultState( void )
{
	...
	glState.renderMode = -1;	// force first GL_SetRenderMode call to apply state
}
...
void R_SaveVideoMode( int w, int h )
{
	...
	// On a non-scaled path the physical display matches the render resolution.
	// Platform backends that support r_scale must set screenWidth/screenHeight
	// separately BEFORE calling R_SaveVideoMode.  Here we only initialise them
	// when they haven't been set yet so we don't overwrite the platform value.
	if( !glState.screenWidth || !glState.screenHeight )
	{
		glState.screenWidth  = w;
		glState.screenHeight = h;
	}
