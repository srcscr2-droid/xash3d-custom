...
{
	if( glState.faceCull == (int)cull )
		return;	// already set — skip redundant driver call
	...
	if( !glState.faceCull )
		pglEnable( GL_CULL_FACE );	// was disabled, need to re-enable
...
{
	if( glState.frontFace == (int)front )
		return;	// already set — skip redundant driver call
...
{
	if( glState.renderMode == mode )
		return;	// already set — avoid redundant GL calls per surface

	// Always set texenv when changing mode (state could be dirty on first call
	// or after a context loss).  Tex-env is cheap compared to blend state changes.
	pglTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );

	// Blend / alpha-test state delta — only touch what changes
	switch( mode )
	{
	case kRenderNormal:
	default:
	...
	case kRenderTransAdd:
		pglEnable( GL_BLEND );
		pglDisable( GL_ALPHA_TEST );
#if defined(XASH_BLEND_ES_WORKAROUND)
